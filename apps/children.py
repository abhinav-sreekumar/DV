...
Crimes_Against_Children = pd.read_csv('Test_CSVs/Crimes_Against_Children_Test.csv')
Police_Strength = pd.read_csv('Test_CSVs/Police_Strength_Test.csv')

x = Crimes_Against_Children.AREA_NAME.unique()
options = []
for i in x:
    options.append({'label': i, 'value': i})

crimeOptions=[]
for i in Crimes_Against_Children.columns[2:]:
    crimeOptions.append({'label': i, 'value': i})

years = []
for i in Crimes_Against_Children.YEAR.unique():
    years.append({'label': i, 'value': i})

caipc = Crimes_Against_Children
...
layout = html.Div([
    ...
    html.Div([
        dcc.Graph(id="children1")
    ], style={'margin': '20px'}),

    html.Div([
        dcc.Graph(id="children2")
    ], style={'margin': '20px'}),
    ...
    html.Div([
        dcc.Graph(id="children3")
    ], style={'display': 'flex', 'justify-content': 'center'}),
])

@app.callback(
    [
        Output('children1', 'figure'),
        Output('children2', 'figure'),
        Output('children3', 'figure')
    ], 
    [
        Input('state', 'value'),
        Input('crime', 'value'),
...
def update_figure(value, crimeValue, yearValue):
    ...
    fig.add_trace(
        go.Bar(
            x=Crimes_Against_Children[Crimes_Against_Children.AREA_NAME.str.contains(value)].YEAR,
            y=Crimes_Against_Children[Crimes_Against_Children.AREA_NAME.str.contains(value)][crimeValue],
    ...
    caipc2 = Crimes_Against_Children
    caipc2 = caipc2[caipc2['YEAR'] == int(yearValue)]
    caipc2 = caipc2[caipc2['AREA_NAME'] == value]

    caipc2.drop("TOTAL", axis=1, inplace=True)
    test = caipc2.columns[3:]
    ...
    df3 = pd.DataFrame({'crimeOptions': test, 'numbers': caipc2.iloc[:, 3:].values[0]})
