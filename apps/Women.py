...
Crimes_Against_Women = pd.read_csv('Test_CSVs/Crimes_Against_Women_Test.csv')
Police_Strength = pd.read_csv('Test_CSVs/Police_Strength_Test.csv')

x = Crimes_Against_Women.AREA_NAME.unique()
options = []
for i in x:
...
@app.callback(
    [
        Output('women1', 'figure'),
        Output('women2', 'figure'),
        Output('women3', 'figure')
    ], 
    [
        Input('state', 'value'),
        Input('crime', 'value'),
...
def update_figure(value, crimeValue, yearValue):
    ...
    fig.add_trace(
        go.Bar(
            x=Crimes_Against_Women[Crimes_Against_Women.AREA_NAME.str.contains(value)].YEAR,
            y=Crimes_Against_Women[Crimes_Against_Women.AREA_NAME.str.contains(value)][crimeValue],
    ...
    caipc2 = Crimes_Against_Women
    caipc2 = caipc2[caipc2['YEAR'] == int(yearValue)]
    caipc2 = caipc2[caipc2['AREA_NAME'] == value]

    caipc2.drop("TOTAL CRIME AGAINST WOMEN", axis=1, inplace=True)
